  ...
  const fetchProfile = async () => {
    try {
      ...
      setCaretaker({
        ...
        role: profileData.role || "Caretaker",  // Add from database
      ...
      localStorage.setItem("caretaker", JSON.stringify(profileData));
    } catch (err) {
...
// ✅ SINGLE OPTIMIZED useEffect - handles profile + stats
useEffect(() => {
  const fetchAllData = async () => {
    try {
      ...
      const [profileRes, interactionRes, todayRes, todayUsersRes] = await Promise.all([
        API.get("/caretaker/profile"),
        API.get("/caretaker/chat/interactions").catch(() => ({ data: { count: 0 } })),
        API.get("/caretaker/chat/interactions/today").catch(() => ({ data: {} })),
        API.get("/caretaker/chat/interactions/today/users").catch(() => ({ data: { userIds: [] } }))
      ]);

      const profileData = profileRes.data;
      
      // ✅ SET CARETAKER (PROFILE PIC FROM DB)
      setCaretaker({
        name: profileData.name || "",
        email: profileData.email || "",
        photo: profileData.photo || "",        // ✅ DATABASE PHOTO
      ...
      setStats({
        ...
        activities: [
          ...(profileData.profile?.activities || []),
      ...
      // ✅ SET CHATTED USERS
      setChattedTodayIds(todayUsersRes.data.userIds || []);
