...
const CaretakerSettings = () => {
  ...
  // ---------------------------
// ✅ CORRECT ENDPOINTS for Caretaker
// ✅ PERFECTLY MATCHES BACKEND ROUTES
const loadData = async () => {
  try {
    ...
    setPhotoValue(profileData.photo || "");

    // Settings  
    const settingsRes = await API.get("/caretaker/settings");
    const settingsData = settingsRes.data;
    setSettings(settingsData);
    
    // Sync theme context
    setTheme(settingsData.theme || "light");
    setColor(settingsData.color || "blue");
    setFont(settingsData.font || "default");
    setAccessibility(settingsData.accessibility || { largeText: false });
...
const handleSaveProfile = async () => {
  try {
    setLoading(true);
    const payload = { name: nameValue.trim(), photo: photoValue };
    
    await API.post("/caretaker/updateProfile", payload); // ✅ Matches backend
    
    // Update localStorage
    const localCaretaker = JSON.parse(localStorage.getItem("caretaker") || "{}");
    localStorage.setItem("caretaker", JSON.stringify({
      ...localCaretaker, 
      name: nameValue.trim(), 
      photo: photoValue
    }));
    
    setProfile({ ...profile, name: nameValue.trim(), photo: photoValue });
...
const handleSaveSettings = async () => {
  try {
    ...
    await API.put("/caretaker/settings", payload); // ✅ Matches backend
...
const handleSaveAll = async () => {
  try {
    ...
    // Profile save
    if (nameValue !== profile.name || photoValue !== profile.photo) {
    ...
    const settingsPayload = {
      theme, color, font, accessibility, notifications: settings.notifications
    };
    await API.put("/caretaker/settings", settingsPayload);
...
const handlePhotoSelect = (file) => {
  ...
  reader.onloadend = () => {
    ...
    // ✅ Fix: use "caretaker" not "user"
    const localCaretaker = JSON.parse(localStorage.getItem("caretaker") || "{}");
    localStorage.setItem("caretaker", JSON.stringify({ ...localCaretaker, photo: base64 }));
  };
...
//     // Primary endpoint
//     await API.post("/api/caretaker/updateProfile", payload);
    
//     // Update localStorage (caretaker key)
//     const localCaretaker = JSON.parse(localStorage.getItem("caretaker") || "{}");
//     const updatedLocal = { ...localCaretaker, name: nameValue.trim(), photo: photoValue };
//     localStorage.setItem("caretaker", JSON.stringify(updatedLocal));
...
// ✅ FIXED: Settings save - matches backend PUT /caretaker/settings
// const handleSaveSettings = async () => {
//   try {
//     setLoading(true);
//     const payload = {
...
//       notifications: settings.notifications
//     };
//     await API.put("/api/caretaker/settings", payload);
...
const handleLogout = () => {
  localStorage.removeItem("token");
  localStorage.removeItem("caretaker"); // ✅ Fixed key
  navigate("/login");
};
