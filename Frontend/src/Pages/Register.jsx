...
useEffect(() => {
  ...
  if (token && user?.isAssessmentCompleted) {
    navigate("/home");
  }
}, [navigate]);
...
const handleRegister = async (e) => {
  ...
  try {
    ...
    if (res.status === 201) {
      // ⛔️ Do NOT log user in automatically here.
      // Just show success and send them to Login.

      toast.success(
        "Registered successfully! Please login to continue 🧠"
      );

      // 👇 Go to login page, login will then decide: assessment or home
      navigate("/login");
    }
  } catch (err) {
    toast.error(
  ...
  onSuccess={async (credentialResponse) => {
    ...
    try {
      ...
      if (!res.data.user.isAssessmentCompleted) {
        navigate("/assessment");
      } else {
        navigate("/home");
      }
