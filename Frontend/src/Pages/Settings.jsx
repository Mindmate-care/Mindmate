...
useEffect(() => {
  const load = async () => {
    try {
      ...
      if (settingsRes?.data) {
        ...
        if (settingsRes.data.color) setColor(settingsRes.data.color);
        if (settingsRes.data.font) setFont(settingsRes.data.font);
        if (settingsRes.data.accessibility) setAccessibility(settingsRes.data.accessibility);
      }
    } catch (err) {
  ...
  load();
}, [navigate, setTheme, setColor, setFont, setAccessibility]);
...
const handleSaveProfile = async () => {
  try {
    ...
    setPhotoValue(response.data.user.photo);
...
// ✅ FIXED: Save Settings - only settings data
