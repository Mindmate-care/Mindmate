...
import { Search, Star, Play, Info, Target } from "lucide-react";
...
const Games = () => {
  ...
  const lighterColor = `${chosenColor}33`; // 20% opacity
  ...
  // read weak areas from user
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user?.weakAreas?.length) {
      setWeakAreas(user.weakAreas.slice(0, 2)); // max 2
    }
  }, []);

  // compute recommended games from weakAreas
  useEffect(() => {
    if (!games.length) {
      setRecommendedGames([]);
      return;
    ...
    // fallback if no weak areas stored
    const effectiveCategories =
      weakAreas.length > 0 ? weakAreas.slice(0, 2) : ["Logic"];

    const recommended = games.filter((game) =>
      effectiveCategories.includes(game.category)
    );

    setRecommendedGames(recommended.slice(0, 6));
  }, [weakAreas, games]);
  ...
  return (
    ...
    >
      <main className="container py-5">
        ...
        {/* 🔹 Recommended Section (above search) */}
        <div
        ...
        >
          <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                <h4 className="fw-bold mb-1" style={{ color: chosenColor }}>
                  Recommended for You
                </h4>
                <p
                ...
                >
                  Practice games that target your current weak areas.
                </p>
            ...
            <div className="text-md-end w-100 w-md-auto">
              {recommendedGames.length > 0 ? (
                <div className="d-flex flex-wrap gap-2 justify-content-start justify-content-md-end mt-2 mt-md-0">
                  {recommendedGames.slice(0, 3).map((game) => (
                    <button
                      key={game._id || game.id}
                      type="button"
                      onClick={() => handleShow(game)}
                      className="btn btn-sm d-flex align-items-center rounded-pill"
                      style={{
                ...
                <span
                  ...
                  style={{ color: isDark ? "#9ca3af" : "#6b7280" }}
                >
                  Complete an assessment to get personalized game suggestions.
                </span>
              )}
            </div>
          </div>
            ...
            <div className="d-flex flex-wrap gap-2 justify-content-center">
              {categories.map((cat) => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      selectedCategory === cat ? chosenColor : "transparent",
                    color:
                      selectedCategory === cat ? "#fff" : chosenColor,
            ...
            {activeGame && (
              <div>
                ...
                <p>
                  ...
                  {activeGame.category || "General"}
                </p>
                <p>
                  <strong>Difficulty:</strong>{" "}
                  {activeGame.difficulty || "N/A"}
                </p>
                <p>
                  <strong>Duration:</strong>{" "}
                  {activeGame.duration || "N/A"}
                </p>
