...
// ✅ ADD THESE MISSING FUNCTIONS (put BEFORE exports)
...
const getSettings = async (req, res) => {
  try {
    const userId = req.user.id || req.user._id;
    const user = await User.findById(userId).select("name email photo settings");
    ...
    res.json({
      ...
      color: user.settings?.color || "blue",
      font: user.settings?.font || "default",
      accessibility: user.settings?.accessibility || { largeText: false },
      notifications: user.settings?.notifications || { email: true, sms: false, push: false }
    });
  } catch (err) {
...
export default {
  ...
  changePassword,
  changeUserName,
  updateBasicProfile,  // Keep your existing one
  
  // ✅ THESE FIXED FUNCTIONS (replace completely)
  getProfile,
  updateProfile,
  getSettings,
  updateSettings,
  
  // Your OTP functions (keep as-is)
  requestPasswordOtp,
